...
var initialSrc = "https://lp.becomingbeautiful.online/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("lp.becomingbeautiful.online/preclick")>-1) {
				...
				xhrr.open("GET", "https://lp.becomingbeautiful.online/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://lp.becomingbeautiful.online/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("lp.becomingbeautiful.online/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
